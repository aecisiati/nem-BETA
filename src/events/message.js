      ...
      if (timestamps.has(message.author.id)) {
        ...
        if (now < expirationTime) {
          ...
          const cooldownEmbed = new Discord.MessageEmbed()
            ...
            .setDescription(
              `This command is on cooldown!\n╰─*You will be able to run this command again in : \`${prettyMilliseconds(
                timeLeft * 1000
              )}\`.*`
            )
            .setColor("#f5da9f");
        ...
        if (!command.config.botPerms)
          return console.log("You didn't provide botPerms");
        if (!Array.isArray(command.config.botPerms))
          return console.log("botPerms must be an array.");
        if (!command.config.userPerms)
          return console.log("You didn't provide userPerms.");
        if (!Array.isArray(command.config.userPerms))
          return console.log("userPerms must be an array.");
